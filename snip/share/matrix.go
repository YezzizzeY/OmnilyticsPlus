...
// Set defines the data of a matrix
func (this *Matrix) Set(m int, n int, data []float64) {
	...
	if len(data) != this.M*this.N {
		...
		if this.M*this.N == len(data) {
			...
			fmt.Println("matrix definition failed")
			return
		}
	}
}

// Mul is multiplication of matrix, M of a must be equal to N of b
func Mul(a Matrix, b Matrix) Matrix {
	if a.M == b.N {
		...
		// "两矩阵⽆法进⾏相乘运算")
		fmt.Println("not able to multiply")
		return Matrix{}
	}
}
